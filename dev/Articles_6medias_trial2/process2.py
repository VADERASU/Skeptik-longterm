    ...
    try:
        with open(file_path, 'r') as file:
            ...
            for case in cases:
                ...
                # Calculate total sentences affected by fallacies
                sentences_affected = set()
                for fallacy, sentences in case.get('fallacies', {}).get('sentences', {}).items():
                    sentences_affected.update(sentences)
                total_sentences_affected = len(sentences_affected)
                case['total_sentences_affected'] = total_sentences_affected

                # Calculate total sentences affected by fallacies normalized by 1000 words
                sentences_affected_per_1000_words = (total_sentences_affected / word_count) * 1000 if word_count != 0 else 0
                case['total_sentences_affected_per_1000_words'] = sentences_affected_per_1000_words
